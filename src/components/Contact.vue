...
<template>
    ...
    border-primary border-2 rounded-3xl inset-x-0 mx-auto backdrop-blur-lg bg-opacity-95 text-center p-5">
        ...
        <form class="m-auto">
            ...
            <input class="primary-input w-full" placeholder="Name" v-model="contactMessage.name" />
            <input class="primary-input w-full" placeholder="Email" v-model="contactMessage.email" />
            <textarea
                class="my-2 bg-transparent rounded-xl border-solid border-primary border-2 p-2 resize-none focus:outline-none w-full h-28"
                placeholder="Message" v-model="contactMessage.content" />
            <button class="primary-button m-auto" v-on:click="createContactMessage"
                @click="$emit('toggle-contact')">Send</button>
        </form>
